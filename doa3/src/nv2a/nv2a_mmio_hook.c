...
static bool decode_and_handle(PCONTEXT ctx, uint32_t mmio_offset, int is_write)
{
    ...
    int has_f2 = 0, has_f3 = 0; /* SSE scalar prefixes */
    ...
    while (1) {
        ...
        if (b == 0x66) {
            ...
            if (b == 0xF2) has_f2 = 1; else has_f3 = 1;
            prefix_len++;
    ...
    /* ── SSE store: MOVSS/MOVSD/MOVUPS [r/m], xmm (0F 11) ──
     * MSVC compiles the recompiled game's MEMF float stores to movss; the
     * D3DX glyph/vertex path writes floats straight into the GPU FIFO
     * window. F3=4-byte movss, F2=8-byte movsd, else 16-byte movups
     * (forwarded as two/four dword writes). */
    if (opcode[0] == 0x0F && opcode[1] == 0x11) {
        int modrm_len = decode_modrm_len(opcode + 2, rex_b);
        int reg = ((opcode[2] >> 3) & 7) | (rex_r ? 8 : 0);
        const uint32_t *x = (const uint32_t *)(&ctx->Xmm0 + reg);
        int ndw = has_f3 ? 1 : (has_f2 ? 2 : 4);
        for (int i = 0; i < ndw; i++)
            nv2a_mmio_write(nv2a, mmio_offset + i * 4, x[i], 4);
        ctx->Rip += prefix_len + 2 + modrm_len;
        g_mmio_write_count++;
        return true;
    }

    /* ── SSE load: MOVSS/MOVSD/MOVUPS xmm, [r/m] (0F 10) ── */
    if (opcode[0] == 0x0F && opcode[1] == 0x10) {
        int modrm_len = decode_modrm_len(opcode + 2, rex_b);
        int reg = ((opcode[2] >> 3) & 7) | (rex_r ? 8 : 0);
        uint32_t *x = (uint32_t *)(&ctx->Xmm0 + reg);
        int ndw = has_f3 ? 1 : (has_f2 ? 2 : 4);
        for (int i = 0; i < ndw; i++)
            x[i] = (uint32_t)nv2a_mmio_read(nv2a, mmio_offset + i * 4, 4);
        ctx->Rip += prefix_len + 2 + modrm_len;
        g_mmio_read_count++;
        return true;
    }
