...
/* DOA3: create/refresh a D3D8 texture from guest RAM (linear formats).
 * Used for the Sofdec movie frame surface and other from-memory textures.
 * Returns NULL if the format isn't a supported linear one. */
extern ptrdiff_t g_xbox_mem_offset;
static IDirect3DTexture8 *get_dynamic_texture(IDirect3DDevice8 *dev)
{
    uint32_t off = g_pg.tex[0].offset;
    uint32_t nvfmt = (g_pg.tex[0].format >> 8) & 0x7F;
    uint32_t w = g_pg.tex[0].image_rect >> 16;
    uint32_t h = g_pg.tex[0].image_rect & 0xFFFF;
    uint32_t pitch = g_pg.tex[0].control1 >> 16;
    uint32_t d3dfmt, bpp;

    if (!off || off >= 0x04000000u) return NULL;
    switch (nvfmt) {
    case 0x12: d3dfmt = 0x12; bpp = 4; break;   /* LIN_A8R8G8B8 */
    case 0x1E: d3dfmt = 0x1E; bpp = 4; break;   /* LIN_X8R8G8B8 */
    case 0x11: d3dfmt = 0x11; bpp = 2; break;   /* LIN_R5G6B5 */
    case 0x10: d3dfmt = 0x10; bpp = 2; break;   /* LIN_A1R5G5B5 */
    default: return NULL;
    }
    if (w == 0 || h == 0 || w > 2048 || h > 2048) return NULL;
    if (pitch < w * bpp || pitch > 0x4000) pitch = w * bpp;  /* control1 unreliable */

    if (g_pg.dyn_tex &&
        (g_pg.dyn_w != w || g_pg.dyn_h != h || g_pg.dyn_fmt != d3dfmt)) {
        g_pg.dyn_tex->lpVtbl->Release(g_pg.dyn_tex);
        g_pg.dyn_tex = NULL;
    }
    if (!g_pg.dyn_tex) {
        if (dev->lpVtbl->CreateTexture(dev, w, h, 1, 0, (D3DFORMAT)d3dfmt,
                                       0, &g_pg.dyn_tex) != 0 || !g_pg.dyn_tex) {
            g_pg.dyn_tex = NULL;
            return NULL;
        }
        g_pg.dyn_w = w; g_pg.dyn_h = h; g_pg.dyn_fmt = d3dfmt;
        fprintf(stderr, "[PGRAPH-D3D11] dynamic texture %ux%u nvfmt=0x%02X pitch=%u off=0x%08X\n",
                w, h, nvfmt, pitch, off);
    }
    /* upload the current guest contents */
    {
        D3DLOCKED_RECT lr;
        if (g_pg.dyn_tex->lpVtbl->LockRect(g_pg.dyn_tex, 0, &lr, NULL, 0) == 0 && lr.pBits) {
            ...
            for (uint32_t yy = 0; yy < h; yy++)
                memcpy(dstp + yy * lr.Pitch, srcp + yy * pitch, row_bytes);
...
int pgraph_d3d11_method(int subchannel, uint32_t method, uint32_t param)
{
    ...
    if (method == 0x0000) {   /* DOA3 DIAG: SET_OBJECT — which class binds where */
        static int s_so = 0;
        if (s_so < 12) {
            s_so++;
            fprintf(stderr, "[PG-OBJ] subch=%d object=0x%08X\n", subchannel, param);
            fflush(stderr);
        }
    }
